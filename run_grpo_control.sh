...
GRPO_DATA="/work/HHRI-AI/POC/public/SFT_Data/mixture_of_thought/All_chunks_split_chunk1"
...
usage() {
  cat <<EOF
Usage:
  $(basename "$0") [--base_model PATH] [--dataset PATH]
                   [--work_dir PATH] [--runs_dir PATH]
                   [--wandb_project NAME] [--run_id ID]
                   [--sequence_len N] [--micro_bsz N] [--grad_accum N]
                   [--epochs N] [--trl_num_gens N] [--trl_max_len N]
                   [--vllm_port PORT]
                   [--server_gpus LIST] [--train_gpus LIST] [--num_processes N]
EOF
}
...
python "${WORK_DIR}/src/config_builders.py" grpo \
  ...
  --wandb_name "qwen3-grpo-run${RUN_ID}"

echo "Merged cfg -> ${MERGED_CFG}"

# -------- Submit the combined sbatch --------
